...
-- 正在导出表  irs2.仪器可用时间段表 的数据：~0 rows (大约)
...
-- 正在导出表  irs2.仪器申请记录表 的数据：~0 rows (大约)
DELETE FROM `仪器申请记录表`;
...
-- 正在导出表  irs2.仪器表 的数据：~3 rows (大约)
DELETE FROM `仪器表`;
/*!40000 ALTER TABLE `仪器表` DISABLE KEYS */;
INSERT INTO `仪器表` (`仪器编号`, `仪器名称`, `型号规格`, `功能描述`) VALUES
	('1001', 'Cup', 'FC02', 'A cup. You can drink water with it. It\'s small.'),
...
-- 正在导出表  irs2.老师资格表 的数据：~2 rows (大约)
DELETE FROM `老师资格表`;
